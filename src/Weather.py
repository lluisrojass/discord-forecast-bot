...
def __get(location, is_metric):
    ...
    query_string = HOST+full_path+'&format=json&callback='
    req = requests.get(query_string)
    results = req.json()['query']['results']
    return results
    ...
    if 'channel' in yql_weather_results:
        ...
        if 'item' in channel:
            ...
            if 'condition' in item:  # current condition & optional title date info
                ...
                message_title += ' ({})\n'.format(condition['date']) if 'date' in condition else '' # date not needed
                try:
                    try:
                        current_conditions += emoji[int(condition['code'])] if 'code' in condition else ''
                    except (ValueError, IndexError):
                        pass
                    current_conditions += '{0} at {1}{2}\n'.format(condition['text'], condition['temp'], unit_system['temperature'])
                except KeyError:
            ...
            if 'forecast' in item:
                try: # today's forecast
                    ...
                    forecast_today += '{0} {1}:\n'.format(forecast['day'], forecast['date']) if 'day' and 'date' in forecast.keys() else '\n'
